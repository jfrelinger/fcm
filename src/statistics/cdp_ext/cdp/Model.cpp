  ...
  while (!theFile.eof()) {
	...
	if (Name == "" && Value == "") {
	  ...
	  if (name == "datafile") {
	    ...
	    mnPrintout = ToInt(value);
	  } else if (name == "gpunchunksize") {
	    mnGPU_Sample_Chunk_Size = ToInt(value);
	  } else if (name == "nu") {
	    mdnu = ToDouble(value);
	  } else if (name == "nu0") {
	    ...
	    mnCompSwaps=ToInt(value);
	  } else if (name=="device") {
	    startDevice = (ToInt(value)-1);
	  } else if (name=="percenterr") {
...
bool Model::Print(){
	...
	cout << "algorithm = " << mstralgorithm << endl;
...
bool Model::Save(string FileName){
	...
	theFile << "seed = " << mnSeed << endl;
	theFile << "swaps = " << mnCompSwaps << endl << endl;
