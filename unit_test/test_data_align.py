...
class DiagAlignTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.alignS = DiagonalAlignDataS(self.x, k=8, size=1000)
        self.align = DiagonalAlignData(self.x, k=8, size=1000)
    ...
    def testDiagAlign(self):
        ...
        npt.assert_array_almost_equal(a, np.eye(4), decimal=1)
        npt.assert_array_almost_equal(b, -10*np.ones(4), decimal=1)
    
    def testDiagAlignS(self):
        a,b = self.alignS.align(self.y, np.ones(8))
        npt.assert_array_almost_equal(a, np.eye(4), decimal=1)
        npt.assert_array_almost_equal(b, -10*np.ones(4), decimal=1)
