...
class ModalDp_clusterTestCase(unittest.TestCase):
    ...
    def testmixprob(self):
        ...
        assert self.mix.prob(pnt)[0] == (self.clst1.prob(
            pnt) + self.clst2.prob(pnt)), 'mixture generates different prob then compoent 1'
        assert self.mix.prob(pnt)[1] == self.clst3.prob(
            pnt), 'mixture generates different prob then compoent 2'

    def testmixproblogged(self):
        pnt = array([1, 1, 1])
        assert self.mix.prob(pnt, logged=True)[0] == logsumexp([self.clst1.prob(
            pnt, logged=True), self.clst2.prob(pnt, logged=True)]), 'mixture generates different prob then compoent 1'
        assert self.mix.prob(pnt)[1] == self.clst3.prob(
            pnt), 'mixture generates different prob then compoent 2'

    def testclassify(self):
        pnt = array([self.mu1, self.mu2])
        assert self.mix.classify(array([self.mu1, self.mu2, self.mu1, self.mu2, self.mu1, self.mu2])).tolist(
        ) == [0, 1, 0, 1, 0, 1], 'classify not working'
    ...
    def testarith(self):
        ...
        self.assertIsInstance(
            b, ModalDPMixture, 'integer addition return wrong type')
        ...
        self.assertIsInstance(
            c, ModalDPMixture, 'array addition return wrong type')
        ...
        b = self.mix - adder
        self.assertIsInstance(
            b, ModalDPMixture, 'integer subtraction return wrong type')
        assert_array_equal(b.mus[0], self.mix.mus[0] - adder,
                           'integer subtraction returned wrong value')

        c = self.mix - array_adder
        self.assertIsInstance(
            c, ModalDPMixture, 'array subtraction return wrong type')
        assert_array_equal(c.mus[0], self.mix.mus[0] - array_adder,
        ...
        b = self.mix * adder
        self.assertIsInstance(
            b, ModalDPMixture, 'integer multiplication return wrong type')
        assert_array_equal(b.mus[0], self.mix.mus[0] * adder,
                           'integer multiplication returned wrong value')

        c = self.mix * array_adder
        self.assertIsInstance(
            c, ModalDPMixture, 'array multiplicaton return wrong type')
        assert_array_equal(c.mus[0], dot(self.mix.mus[0], array_adder),
                           'array multiplication returned wrong value')

        d = self.mix * mat_adder
        self.assertIsInstance(
            d, ModalDPMixture, 'array multiplicaton return wrong type')
        ...
        c = array_adder * self.mix
        self.assertIsInstance(
            c, ModalDPMixture, 'array multiplication return wrong type')
