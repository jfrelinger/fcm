...
class mvnpdfTestCase(unittest.TestCase):
    ...
    def testSinglePointSingleComponent_1d(self):
        ...
        mu = array([1])
        sigma = eye(1)
        self.assertAlmostEqual(pdf(x, mu, sigma), mvnormpdf(
            x, mu, sigma), delta=1e-6, msg='pmvnormpdf and mvnormpdf differ in result, %f != %f' % (pdf(x, mu, sigma), mvnormpdf(x, mu, sigma)))
        for i in range(10):
            x = array([uniform(-4, 4)])
            ...
            self.assertAlmostEqual(p, m, delta=1e-6, msg='pmvnormpdf and mvnormpdf differ in result, %0.8e != %0.8e, [%0.8e], (%d): %s, %s, %s ' % (
    ...
    def testMultiplePointSingleComponent_1d(self):
        ...
        assert_array_almost_equal(px, mx, 6, 'pmvnormpdf and mvnormpdf differ in result, %s != %s' % (
            str(type(px)), str(type(mx))))
    ...
    def testSinglePointSingleComponent(self):
        ...
        self.assertAlmostEqual(
            px, mx, 6, 'pmvnormpdf and mvnormpdf differ in result, %f != %f' % (px, mx))
        for i in range(100):
            ...
            mx = mvnormpdf(x, mu, sigma)
            px = pmvnormpdf(x, mu.squeeze(), sigma.squeeze())
            self.assertAlmostEqual(px, mx, 6, 'pmvnormpdf and mvnormpdf differ in result, %f != %f, (%d): %s, %s, %s ' % (
                px, mx, i, str(x), str(mu), str(sigma).replace('\n', ',')))

    def testMultiplePointSingleComponent(self):
        for i in range(100):
            x = array(
                [[uniform(-4, 4), uniform(-4, 4)], [uniform(-4, 4), uniform(-4, 4)]])
    ...
    def testSinglePointMultipleComponent(self):
        for i in range(100):
            ...
            presult = array([float(pmvnormpdf(x, mu[0, :], sigma[0, :, :])), float(
                pmvnormpdf(x, mu[1, :], sigma[1, :, :]))])
            cresult = compmixnormpdf(x, pi, mu, sigma)
            # print result, compmixnormpdf(x,pi,mu,sigma)
            self.assertAlmostEqual(presult[0], cresult[0], 6, 'pmvnormpdf and mvnormpdf differ in result, %f != %f, (%d): %s, %s, %s ' % (
                presult[0], cresult[0], i, str(x), str(mu[0]), str(sigma[0]).replace('\n', ',')))
            self.assertAlmostEqual(presult[1], cresult[1], 6, 'pmvnormpdf and mvnormpdf differ in result, %f != %f, (%d): %s, %s, %s ' % (
                presult[1], cresult[1], i, str(x), str(mu[1]), str(sigma[1]).replace('\n', ',')))
