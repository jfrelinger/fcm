...
class HDPMixtureTestCase(unittest.TestCase):
    ...
    def test_arith(self):
        ...
        numpy.testing.assert_array_equal(
            (self.mix * eye(2)).sigmas, self.mix.sigmas, 'multicplication failed')
    ...
    def test_getitem(self):
        ...
        numpy.testing.assert_array_equal(
            self.mix[0].pis, self.pis[0, :], "returned pis wrong")
    ...
    def test_stats(self):
        ...
        self.assertEqual(
            classified.shape, (3,), 'probability return wrong shape,')

    def test_average(self):
        avg = self.mix.average()
        numpy.testing.assert_array_equal(
            avg.mus, self.mix.mus, 'averaging got means wrong')
        numpy.testing.assert_array_equal(
            avg.sigmas, self.mix.sigmas, 'averaging got variance wrong')
        self.assertIsInstance(
            avg, HDPMixture, 'average returned wrong type of object')
        self.assertEqual(
            len(avg), 3, 'average return the wrong number of mixtures, %d' % len(avg))

    def test_make_modal(self):
        modal = self.mix.make_modal()
        self.assertIsInstance(
            modal, ModalHDPMixture, 'make_modal failed to return ModalHDPMixture object')
