...
class FCMcollectionTestCase(unittest.TestCase):
    ...
    def testTransform(self):
        ...
        assert_array_equal(
            self.fcms['test_fcm1'], ltmp, 'Log transform failed')
        assert_array_equal(
            self.fcms['test_fcm2'], ltmp, 'Log transform failed')
    ...
    def testGate(self):
        ...
        self.fcms.gate(g)
        assert_array_equal(
            self.fcms['test_fcm1'].view(), array([[1, 1, 1]]), 'Gating failed')
        assert_array_equal(
            self.fcms['test_fcm2'].view(), array([[1, 1, 1]]), 'Gating failed')
    ...
    def testFit(self):
        ...
        assert_equal(
            len(results.keys()), 2, 'fitting produced wrong shaped dict')
        for i in self.fcms:

            assert isinstance(
                results[i], DPMixture), 'fitting produced a non-DPMixture object'
