...
class FCMdataTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fcm = FCMdata(
            'test_fcm', self.pnts, [('fsc', 'fsc'), ('ssc', 'ssc'), ('fl-1', 'cd3')], [0, 1])
    ...
    def testLongNames(self):
        assert self.fcm.long_names[
            2] == 'fl-1::cd3', 'long names property fails: %s' % self.fcm.long_names[2]
        assert self.fcm.long_names[
            0] == 'fsc', 'long names property fails: %s' % self.fcm.long_names[0]
    ...
    def testGetChannelByName(self):
        ...
        assert self.fcm.get_channel_by_name(
            ['fl-1'])[0] == 2, 'incorrect last column: %d' % self.fcm.get_channel_by_name(['fl-1'])[0]
        assert self.fcm.get_channel_by_name(
            ['fl-1'])[1] == 5, 'incorrect first column: %d' % self.fcm.get_channel_by_name(['fl-1'])[1]
    ...
    def testGetItem(self):
        a = randint(0, 1)
        b = randint(0, 2)
        assert isinstance(self.fcm[a], type(
            self.pnts[a])), "__getitem__ failed to return array"

        assert self.fcm[a, b] == self.pnts[
            a, b], '__getitem__ returned wrong value'
...
#        self.fcm = FCMdata(d3, ['a'])
    ...
    def testPolyGate(self):
        ...
        self.fcm.gate(g)
        assert all(self.fcm.view() == array(
            [[0, 1, 2]])), 'gate excluded wrong points'
        self.fcm.visit('root')
        self.fcm.gate(g)
    ...
    def testEmptyPolyGate(self):
        ...
        self.fcm.gate(g)
        assert_array_equal(
            self.fcm.view(), array([]).reshape((0, 3)), 'gated region not empty')
    ...
    def testIntervalGate(self):
        ...
        self.fcm.gate(g)
        assert_array_equal(
            self.fcm.view(), array([[3, 4, 5]]), 'gate excluded wrong points')
        self.fcm.visit('root')
        self.fcm.gate(g)
        nodes = self.fcm.tree.nodes.keys()
        assert 'g2' in nodes, 'gating name mangled'
    ...
    def testBoundaryEvents(self):
        ...
        fcm = FCMdata(
            'test_fcm', pnts, [('fsc', 'fsc'), ('ssc', 'ssc'), ('fl-1', 'cd3')], [0, 1])
        eps = 1e-10
    ...
    def testChainOp(self):
        ...
        self.assertTrue(
            all(self.fcm.view() == array([[0, 1, 2]])), 'gate excluded wrong points')
    ...
    def testCopy(self):
        cpy = self.fcm.copy()
        self.assertFalse(
            cpy is self.fcm, "copy reproduced the exact same object")
    ...
    def testRandomSubsample(self):
        ...
        self.assertTrue(
            self.fcm[0] in self.pnts, 'random subsample geneterated non-existant point')
    ...
    def testAnomalySubsample(self):
        ...
        self.assertTrue(
            self.fcm[0] in self.pnts, 'anomaly subsample geneterated non-existant point')
    ...
    def testBiasSubsample(self):
        ...
        self.assertTrue(
            self.fcm[0] in self.pnts, 'bias subsample geneterated non-existant point')

    def testSubsamplebySlice(self):
        self.fcm.subsample(slice(1))
        self.assertEqual(1, self.fcm.shape[0])
        assert_array_equal(
            self.pnts[0], self.fcm[0], 'subsample by slice failed')
